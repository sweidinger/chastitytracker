...
export async function createVerschlussAnforderung(
  ...
  try {
    anforderung = await prisma.$transaction(async (tx) => {
      ...
      // Der Ist-Zustand wird nur gegen eine SOFORT wirksame Direktive geprüft. Eine TERMINIERTE
      // sagt nichts über jetzt, sondern über später: „schliess dich morgen früh ein" ist auch dann
      // sinnvoll, wenn der Sub gerade noch verschlossen/tragend ist. Ob die Auslösung dann noch
      // passt, entscheidet der Poller (processDueVerschlussAnforderungen) — er zieht sie als
      // `obsolete` zurück, statt eine unpassende Anweisung zuzustellen.
      if (isPlugCategory) {
      ...
      // Nur die SPERRZEIT ist exklusiv: eine neue Sperre ERSETZT die bestehende (sonst konkurrierende
      // Enden, von denen `foldActiveSperrzeiten` stumm das spätere durchsetzte). ANFORDERUNGen dürfen
      // koexistieren (Upstream v4.52.0: mehrere Einschliess-Anweisungen als Pipeline, ein Verschluss
      // erfüllt sie alle — siehe POST /api/entries). Fork-Scoping bleibt: pro deviceCategoryId
      // (Körperregion), damit KG- und Plug-Sperren unabhängig voneinander sind.
      if (art === "SPERRZEIT") {
      ...
      // entsteht — also mit dauerH ODER absolutem sperrEndetAt. Fuer den Plug gibt es weder
      // Reinigungs- noch Toilettenfreigabe.
  ...
  if (!wirksamAb) {
    await sendVerschlussAnforderungNotifications({ userId, user, art, nachricht, endetAtDate, dauerH, sperrEndetAtDate, isPlugCategory, requestId: anforderung.id });
  }
...
export async function sendVerschlussAnforderungNotifications(opts: {
  ...
  const { userId, user, art, nachricht, endetAtDate, dauerH, sperrEndetAtDate, isPlugCategory, requestId } = opts;
