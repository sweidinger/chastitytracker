...
export interface JudgeOffenseArgs {
  ...
  text?: string;
  /** Optionale Straf-Aktion (Fork, Phase 3): wird bei action="punish" direkt ausgefuehrt. */
...
export async function mcpJudgeOffense(username: string, args: JudgeOffenseArgs) {
  ...
  if (args.dryRun) {
    ...
    const after: Record<string, unknown> | undefined = !knownTransition ? undefined
      ...
      : { status: judgmentStatus(args.action === "punish" ? "punish" : "dismiss"), reason: args.text?.trim() || null, judgedBy: "ai", erledigtAt: null };
