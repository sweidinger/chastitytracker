...
export async function GET(
  ...
  const [entryOwner, deviceOwner, codePhoto, refOwner, strafeOwner, boxPhotoOwner] = await Promise.all([
  ...
  const ownerId = entryOwner?.userId ?? deviceOwner?.userId ?? codePhoto?.userId ?? refOwner?.device?.userId ?? strafeOwner?.userId ?? boxPhotoOwner?.userId ?? null;
