...
export default function ToiletteToggle({
  userId,
  initialErlaubt,
  initialMaxMinuten,
  initialMaxProTag,
  ...
  async function save(newErlaubt: boolean, newMaxMin: number, newMaxProTag: number) {
    ...
    await fetch(`/api/admin/users/${userId}`, {
      ...
      body: JSON.stringify({ toiletteErlaubt: newErlaubt, toiletteMaxMinuten: newMaxMin, toiletteMaxProTag: newMaxProTag }),
    });
    setSaving(false);
    router.refresh();
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <Toggle
        label={t("toilettePausenLabel")}
        description={t("toilettePausenDesc")}
        checked={erlaubt}
        disabled={saving}
      ...
      {erlaubt && (
        <>
          ...
          <div className="flex items-center gap-2 pl-1">
            <span className="text-xs text-foreground-faint">{t("toiletteMaxProTagLabel")}</span>
            <input
              type="number"
              min={0}
              max={20}
